...
def multiples_of_ten(start, stop):
  ...
  for i in range(start, stop):
    if (i % 10) == 0:
        ...
        break
  if i % 10 != 0:
    x = torch.arange(0, 0, 10, dtype=torch.float64)
    return x
...
def reverse_rows(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  idx0 = torch.flip(torch.arange(x.shape[0]), [0])
  y = x[idx0, :]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def reshape_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  x_reshaped = x.contiguous().view(2, 3, 4)
  x_transposed = x_reshaped.transpose(0, 1)
  y = x_transposed.contiguous().view(3,8)  
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def zero_row_min(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  row_min_vals, row_min_indxs = x.min(dim=1)
  y = x.contiguous().view(x.shape)
  y = y.t()
  y = y-(row_min_vals)
  mask = (y != 0)
  y = (mask * row_min_vals) + y
  y = y.t()
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def normalize_columns(x):
  ...
  # Replace "pass" statement with your code
  y = x.contiguous().view(x.shape)
  stds = torch.std(y, dim=0)
  means = torch.mean(y, dim=0)
  y = y - means
  y = y / stds
  #############################################################################
  #                            END OF YOUR CODE                               #
